...
const encrypt = async (input: string) => {
  ...
  const cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(config.SecretsKey), iv);
...
const decrypt = async(input: string, iv: Buffer) => {
  const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(config.SecretsKey), iv);
  const decrypted = decipher.update(input, 'hex', 'utf-8') + decipher.final('utf-8');
  return decrypted;
}
